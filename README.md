# mtulivu.com

Guru page for [mtulivu.com](https://mtulivu.com) and [www.mtulivu.com](https://www.mtulivu.com).

- Photo: `gfx-guru-sam/me-myself-and-guru-sam.png` (center of the screen)
- 999 original peace / meditation lines in comic text bubbles from the head (new line every 3–5 seconds)
- Seasons loop every 15s: summer → fall → winter → spring, with forest, lake, mountains, weather, and flying butterflies
- Mail: [hello@mtulivu.com](mailto:hello@mtulivu.com)

This repo’s GitHub Pages train is **kcal.lol**. Do not point this branch at the kcal Pages environment.

To host the domain, publish **this folder** as its own GitHub Pages site (dedicated repo) with custom domain `mtulivu.com` (www as alias). Apex + www already resolve to GitHub Pages IPs.
